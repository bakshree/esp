...
#include "rt_accel_functions.hpp"

// Processes

void rt_accel::load_input()
{
...
#else
            uint32_t length = round_up(3*img_width*img_height, DMA_WORD_PER_BEAT);
#endif
            // Chunking
...
void rt_accel::store_output()
{
    ...
    // Store
    {
        ...
        // bool ping = true;
#if (DMA_WORD_PER_BEAT == 0)
        uint32_t store_offset = (3*img_width*img_height) * 1;
#else
        uint32_t store_offset = round_up(3*img_width*img_height, DMA_WORD_PER_BEAT) * 1;
#endif
...
void rt_accel::compute_kernel()
{
    ...
    // bool ping = true;
    {
        for (uint16_t b = 0; b < 1; b++)
        {
            uint32_t in_length = 3*img_width*img_height;
            uint32_t out_length = 3*img_width*img_height;
            int out_rem = out_length;

            for (int in_rem = in_length; in_rem > 0; in_rem -= PLM_IN_WORD) // A min of 40x40 image
            {
