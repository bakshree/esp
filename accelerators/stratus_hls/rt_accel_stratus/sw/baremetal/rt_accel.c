...
#define SLD_RT_ACCEL 0x027
#define DEV_NAME "sld,rt_accel_stratus"

/* <<--params-->> */
const int32_t img_width = 40;
const int32_t img_height = 40;
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_RT_ACCEL, DEV_NAME);
	if (ndev == 0) {
		printf("rt_accel not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, RT_ACCEL_IMG_WIDTH_REG, img_width);
		iowrite32(dev, RT_ACCEL_IMG_HEIGHT_REG, img_height);
