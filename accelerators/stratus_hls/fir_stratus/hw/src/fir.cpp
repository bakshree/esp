        ...
        //for (uint16_t b = 0; b < 1; b++)
        {
            ...
            //for (int rem = length; rem > 0; rem -= PLM_IN_WORD)
            {
                ...
                for (uint16_t i = 0; i < length; i++)
                {
                    ...
                    // Write to PLM
                    if (i<PLM_OUT_WORD)
                        fir_filter[i] = dataBv.to_int64();
                    else
                        super_twiddle[i-PLM_OUT_WORD] = dataBv.to_int64();
                }
...
void fir::compute_kernel()
{
    ...
    // freqdata[0].r = tdc.r + tdc.i;
                    ...
                    for (int k = 1; k <= (out_length/2); k++) { 
                        ...
                        int64_t supt_im = super_twiddle[k-1];
                        int32_t supt_re; 
                        supt_re = supt_im>>32; 
                        supt_im = supt_im&0xFFFFFFFF;

                        int32_t tw_i, tw_r;
                        tw_r = f2k_r*supt_re - f2k_i*supt_im;
                        tw_i = f2k_r*supt_im - f2k_i*supt_re;
    ...
    // st->tmpbuf[0].i = freqdata[0].r - freqdata[ncfft].r;
                    ...
                    for (int k = 1; k <= (out_length/2); k++) { 
                        ...
                        fok_r = tmp_r*supt_re - tmp_i*supt_im;
                        fok_i = tmp_r*supt_im - tmp_i*supt_re;
