...
#define SLD_TILED_ACC 0x027
#define DEV_NAME "sld,tiled_acc_stratus"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_TILED_ACC, DEV_NAME);
	if (ndev == 0) {
		printf("tiled_acc not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, TILED_ACC_NUM_TILES_REG, num_tiles);
		iowrite32(dev, TILED_ACC_TILE_SIZE_REG, tile_size);
